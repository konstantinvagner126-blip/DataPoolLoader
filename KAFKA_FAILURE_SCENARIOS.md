...
### 5.5. Produce on readOnly cluster

`readOnly=true` — это safety contract, а не cosmetic badge.

Если cluster помечен как `readOnly`, produce path должен быть запрещен до Kafka client call.

### 5.6. Kafka UI state must not become a second config source
