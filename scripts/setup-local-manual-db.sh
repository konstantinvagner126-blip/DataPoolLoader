...
SQL_FILE="$ROOT_DIR/scripts/local-postgres-manual-setup.sql"
...
echo "Подготовка схемы datapool_manual в $PGDATABASE@$PGHOST:$PGPORT пользователем $PGUSER"
"$PSQL_BIN" -h "$PGHOST" -p "$PGPORT" -U "$PGUSER" -d "$PGDATABASE" -f "$SQL_FILE"
echo "Схема datapool_manual подготовлена."
