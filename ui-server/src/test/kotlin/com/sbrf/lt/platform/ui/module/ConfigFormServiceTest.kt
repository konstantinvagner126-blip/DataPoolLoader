...
class ConfigFormServiceTest {
    ...
    fun `applies visual form changes without dropping unsupported config blocks`() {
        ...
        assertTrue(result.configText.contains("commonSqlFile: classpath:sql/override.sql"), result.configText)
        assertTrue(result.configText.contains("sources:"), result.configText)
        assertTrue(result.configText.contains("mergeMode: quota"), result.configText)
        assertTrue(result.configText.contains("errorMode: continue_on_error"), result.configText)
        assertTrue(result.configText.contains("commonSql: select 1 as id"), result.configText)
        assertTrue(result.configText.contains("sqlFile: classpath:sql/db1.sql"), result.configText)
        assertTrue(result.configText.contains("quotas:"), result.configText)
        assertTrue(result.configText.contains("parallelism: 7"), result.configText)
        assertTrue(result.configText.contains("table: public.new_table"), result.configText)
